...
use crate::instructions::calculate_accrued_interest;
use crate::state::*;
...
pub fn process_liquidate(ctx: Context<Liquidate>) -> Result<()> { 
    ...
    match ctx.accounts.collateral_mint.to_account_info().key() {
        key if key == user.usdc_address =>{
            ...
            let new_sol= calculate_accrued_interest(user.deposited_sol, borrowed_bank.interest_rate, user.last_updated)?;
            total_borrowed=sol_price.price as u64  * new_sol;
        }
        _=>{

            let new_usdc = calculate_accrued_interest(user.borrowed_usdc, borrowed_bank.interest_rate, user.last_updated)?;
